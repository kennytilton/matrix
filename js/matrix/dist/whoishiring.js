...
goog.addDependency = function(c, d, e, f) {
  if (goog.DEPENDENCIES_ENABLED) {
    var g = goog.getLoader_();
    g && g.addDependency(c, d, e, f);
  }
};
...
goog.require = function(c) {
  goog.ENABLE_DEBUG_LOADER && goog.debugLoader_ && goog.getLoader_().earlyProcessLoad(c);
  if (!COMPILED) {
    if (goog.isProvided_(c)) {
      if (goog.isInModuleLoader_()) {
        return goog.module.getInternal_(c);
      ...
      if (goog.ENABLE_DEBUG_LOADER) {
        ...
        try {
          ...
          e ? e.load(c) : goog.logToConsole_("Could not load " + c + " because there is no debug loader.");
        } finally {
...
goog.loadModule = function(c) {
  ...
  try {
    ...
    if (goog.isFunction(c)) {
      ...
      if (goog.isString(c)) {
        goog.useSafari10Workaround() && (c = goog.workaroundSafari10EvalBug(c)), e = goog.loadModuleFromSource_.call(void 0, c);
      } else {
        throw Error("Invalid module definition");
      }
    }
    var f = goog.moduleLoaderState_.moduleName;
    if (goog.isString(f) && f) {
      goog.moduleLoaderState_.declareLegacyNamespace ? goog.constructNamespace_(f, e) : goog.SEAL_MODULE_EXPORTS && Object.seal && "object" == typeof e && null != e && Object.seal(e), goog.loadedModules_[f] = e;
    } else {
...
goog.DEPENDENCIES_ENABLED && (goog.inHtmlDocument_ = function() {
  ...
  return e;
}, goog.Transpiler.prototype.needsTranspile = function(c) {
...
}, goog.DebugLoader.IS_OLD_IE_ = !(goog.global.atob || !goog.global.document || !goog.global.document.all), goog.DebugLoader.prototype.earlyProcessLoad = function(c) {
  goog.DebugLoader.IS_OLD_IE_ && this.maybeProcessDeferredDep_(c);
}, goog.DebugLoader.prototype.load = function(c) {
  var d = this.getPathFromDeps_(c);
  if (d) {
    var e = [], f = {}, g = this.dependencies_, h = this, k = function(c) {
      if (!(c in g.written || c in g.visited)) {
        g.visited[c] = !0;
        if (c in g.requires) {
          for (var d in g.requires[c]) {
            if (!h.isProvided(d)) {
              if (d in g.nameToPath) {
                k(g.nameToPath[d]);
              } else {
        ...
        c in f || (f[c] = !0, e.push(c));
      }
    };
    k(d);
    for (c = 0; c < e.length; c++) {
      d = e[c], this.dependencies_.written[d] = !0;
    }
    for (c = 0; c < e.length; c++) {
      if (d = e[c]) {
        var l = g.loadFlags[d] || {}, m = l.lang || "es3";
        m = this.getTranspiler().needsTranspile(m);
        "goog" == l.module || m ? this.importProcessedScript_(goog.basePath + d, "goog" == l.module, m) : this.importScript_(goog.basePath + d);
      } else {
        throw Error("Undefined script input");
      }
    }
  } else {
    throw c = "goog.require could not find: " + c, this.logToConsole(c), Error(c);
  }
}, goog.DebugLoader.prototype.addDependency = function(c, d, e, f) {
  var g;
  c = c.replace(/\\/g, "/");
  var h = this.dependencies_;
  f && "boolean" !== typeof f || (f = f ? {module:"goog"} : {});
  for (var k = 0; g = d[k]; k++) {
    h.nameToPath[g] = c, h.loadFlags[c] = f;
  }
  for (f = 0; d = e[f]; f++) {
    c in h.requires || (h.requires[c] = {}), h.requires[c][d] = !0;
  }
}, goog.DebugLoader.prototype.importScript_ = function(c, d) {
  (goog.global.CLOSURE_IMPORT_SCRIPT || goog.bind(this.writeScriptTag_, this))(c, d) && (this.dependencies_.written[c] = !0);
}, goog.DebugLoader.prototype.importProcessedScript_ = function(c, d, e) {
  this.importScript_("", 'goog.debugLoader_.retrieveAndExec_("' + c + '", ' + d + ", " + e + ");");
}, goog.DebugLoader.prototype.retrieveAndExec_ = function(c, d, e) {
  if (!COMPILED) {
    var f = c;
    c = this.normalizePath(c);
    var g = goog.global.CLOSURE_IMPORT_SCRIPT || goog.bind(this.writeScriptTag_, this), h = this.loadFileSync(c);
    if (null == h) {
      throw Error('Load of "' + c + '" failed');
    }
    e && (h = this.getTranspiler().transpile(h, c));
    h = d ? this.wrapModule_(c, h) : h + ("\n//# sourceURL=" + c);
    goog.DebugLoader.IS_OLD_IE_ && this.oldIeWaiting_ ? (this.dependencies_.deferred[f] = h, this.queuedModules_.push(f)) : g(c, h);
  }
}, goog.DebugLoader.prototype.wrapModule_ = function(c, d) {
  return goog.LOAD_MODULE_USING_EVAL && goog.isDef(goog.global.JSON) ? "goog.loadModule(" + goog.global.JSON.stringify(d + "\n//# sourceURL=" + c + "\n") + ");" : 'goog.loadModule(function(exports) {"use strict";' + d + "\n;return exports});\n//# sourceURL=" + c + "\n";
}, goog.DebugLoader.prototype.loadQueuedModules_ = function() {
  var c = this.queuedModules_.length;
  if (0 < c) {
    var d = this.queuedModules_;
    this.queuedModules_ = [];
    for (var e = 0; e < c; e++) {
      this.maybeProcessDeferredPath_(d[e]);
    }
  }
  this.oldIeWaiting_ = !1;
}, goog.DebugLoader.prototype.maybeProcessDeferredDep_ = function(c) {
  this.isDeferredModule_(c) && this.allDepsAreAvailable_(c) && (c = this.getPathFromDeps_(c), this.maybeProcessDeferredPath_(goog.basePath + c));
}, goog.DebugLoader.prototype.isDeferredModule_ = function(c) {
  var d = (c = this.getPathFromDeps_(c)) && this.dependencies_.loadFlags[c] || {}, e = d.lang || "es3";
  return c && ("goog" == d.module || this.getTranspiler().needsTranspile(e)) ? goog.basePath + c in this.dependencies_.deferred : !1;
}, goog.DebugLoader.prototype.allDepsAreAvailable_ = function(c) {
  if ((c = this.getPathFromDeps_(c)) && c in this.dependencies_.requires) {
    for (var d in this.dependencies_.requires[c]) {
      if (!this.isProvided(d) && !this.isDeferredModule_(d)) {
        return !1;
      }
    }
  }
  return !0;
}, goog.DebugLoader.prototype.maybeProcessDeferredPath_ = function(c) {
  if (c in this.dependencies_.deferred) {
    var d = this.dependencies_.deferred[c];
    delete this.dependencies_.deferred[c];
    goog.globalEval(d);
  }
}, goog.DebugLoader.prototype.writeScriptSrcNode_ = function(c) {
  goog.global.document.write('<script type="text/javascript" src="' + c + '">\x3c/script>');
}, goog.DebugLoader.prototype.appendScriptSrcNode_ = function(c) {
  var d = goog.global.document, e = d.createElement("script");
  e.type = "text/javascript";
  e.src = c;
  e.defer = !1;
  e.async = !1;
  d.head.appendChild(e);
}, goog.DebugLoader.prototype.writeScriptTag_ = function(c, d) {
  if (this.inHtmlDocument()) {
    ...
    if (!goog.ENABLE_CHROME_APP_SAFE_SCRIPT_LOADING && "complete" == e.readyState) {
      ...
      throw Error('Cannot write "' + c + '" after document load');
    }
    void 0 === d ? goog.DebugLoader.IS_OLD_IE_ ? (this.oldIeWaiting_ = !0, d = " onreadystatechange='goog.debugLoader_.onScriptLoad_(this, " + ++this.lastNonModuleScriptIndex_ + ")' ", e.write('<script type="text/javascript" src="' + c + '"' + d + ">\x3c/script>")) : goog.ENABLE_CHROME_APP_SAFE_SCRIPT_LOADING ? this.appendScriptSrcNode_(c) : this.writeScriptSrcNode_(c) : e.write('<script type="text/javascript">' + this.protectScriptTag_(d) + "\x3c/script>");
    return !0;
  }
  return !1;
}, goog.DebugLoader.prototype.protectScriptTag_ = function(c) {
  return c.replace(/<\/(SCRIPT)/ig, "\\x3c/$1");
}, goog.DebugLoader.prototype.onScriptLoad_ = function(c, d) {
  "complete" == c.readyState && this.lastNonModuleScriptIndex_ == d && this.loadQueuedModules_();
  return !0;
}, goog.DebugLoader.prototype.getPathFromDeps_ = function(c) {
  return c in this.dependencies_.nameToPath ? this.dependencies_.nameToPath[c] : null;
}, goog.DebugLoader.prototype.getTranspiler = function() {
  return goog.transpiler_;
}, goog.DebugLoader.prototype.isProvided = function(c) {
  return goog.isProvided_(c);
}, goog.DebugLoader.prototype.inHtmlDocument = function() {
  return goog.inHtmlDocument_();
}, goog.DebugLoader.prototype.logToConsole = function(c) {
  goog.logToConsole_(c);
}, goog.DebugLoader.prototype.loadFileSync = function(c) {
  return goog.loadFileSync_(c);
}, goog.DebugLoader.prototype.normalizePath = function(c) {
  return goog.normalizePath_(c);
}, goog.debugLoader_ = null, goog.registerDebugLoader = function(c) {
  if (goog.debugLoader_) {
    throw Error("Debug loader already registered!");
  }
  if (!(c instanceof goog.DebugLoader)) {
  ...
  if (goog.DEBUG_LOADER) {
    var c = new goog.DebugLoader;
    c.importScript_(goog.basePath + goog.DEBUG_LOADER);
  }
  goog.global.CLOSURE_NO_DEPS || (c = c || new goog.DebugLoader, goog.DEBUG_LOADER || goog.registerDebugLoader(c), c.importScript_(goog.basePath + "deps.js"));
}());
var Matrix = {Cells:{}};
function clg(c) {
...
var MXStorable = function(c) {
  var d = c || {};
  d = Object.assign({id:(d.lsPrefix || "MXSTOR_ANON") + uuidv4(), created:Date.now()}, d, {deleted:d.deleted || cI(null)});
  Model.call(this, null, null, d, !1);
  c.id || this.store();
};
...
MXStorable.countAllItems = function(c) {
  return Object.keys(window.localStorage || {}).filter(function(d) {
...
MXStorable.removeAllItems = function(c) {
  return Object.keys(window.localStorage || {}).filter(function(d) {
    return d.startsWith(c);
  }).map(function(c) {
...
goog.array.binarySearch_ = function(c, d, e, f, g) {
  for (var h = 0, k = c.length, l; h < k;) {
    var m = h + k >> 1;
    var r = e ? d.call(g, c[m], m, c) : d(f, c[m]);
    0 < r ? h = m + 1 : (k = m, l = !r);
  }
  return l ? h : ~h;
};
...
goog.debug.catchErrors = function(c, d, e) {
  ...
  e.onerror = function(d, e, l, m, r) {
    f && f(d, e, l, m, r);
    c({message:d, fileName:e, line:l, lineNumber:l, col:m, error:r});
    return g;
  };
};
          ...
          if (goog.isString(c)) {
            ...
            if (goog.isFunction(c)) {
              ...
              if (goog.isObject(c)) {
                ...
                var r = goog.getUid(c);
                if (g[r]) {
                  e.push("*** reference loop detected (id=" + r + ") ***");
                } else {
                  g[r] = !0;
                  e.push("{");
                  for (var t in c) {
                    if (d || !goog.isFunction(c[t])) {
                      e.push("\n"), e.push(k), e.push(t + " = "), h(c[t], k);
                    }
                  }
                  e.push("\n" + l + "}");
                  delete g[r];
...
goog.math.longestCommonSubsequence = function(c, d, e, f) {
  ...
  l = g;
  for (m = h; 0 < l && 0 < m;) {
    e(c[l - 1], d[m - 1]) ? (r.unshift(f(l - 1, m - 1)), l--, m--) : k[l - 1][m] > k[l][m - 1] ? l-- : m--;
  }
  return r;
};
...
;var Hiring = {usernote:{}}, JOB_LS_PREFIX = "whoishiring.", UserNotes = function(c) {
  MXStorable.call(this, Object.assign({lsPrefix:JOB_LS_PREFIX}, c, {hnId:c.hnId, stars:cI(c.stars), hidden:cI(c.hidden || !1), applied:cI(c.applied || !1), notes:cI(c.notes)}));
};
$jscomp.inherits(UserNotes, MXStorable);
UserNotes.cname = MXStorable.cname;
UserNotes.removeAllItems = MXStorable.removeAllItems;
UserNotes.loadAllItems = MXStorable.loadAllItems;
UserNotes.countAllItems = MXStorable.countAllItems;
UserNotes.storeObject = MXStorable.storeObject;
UserNotes.obsAnyChange = MXStorable.obsAnyChange;
UserNotes.load = MXStorable.load;
UserNotes.storableProperties = function() {
  return "id hnId stars hidden applied notes".split(" ").concat(MXStorable.storableProperties.call(this));
};
UserNotes.loadFromStorage = function() {
  return mkm(null, "JobList", {dict:cF(function(c) {
    c = {};
    for (var d = MXStorable.loadAllItems(UserNotes, JOB_LS_PREFIX) || [], e = 0; e < d.length; ++e) {
      var f = d[e];
      c[f.hnId] = f;
    }
    return c;
...
function parseListings(c, d, e, f) {
  var g = c.length, h = function(k) {
    var l = Math.min(g - k, e);
    if (0 < l) {
      for (jn = 0; jn < l; ++jn) {
        var m = jobSpec(c[k + jn]);
        if (m.OK) {
          var r = m.hnId;
          UJob.dict[r] || (UJob.dict[r] = new UserNotes({hnId:r}));
          d.push(m);
        }
      }
      f.value += 1;
      ...
      f.hidden = !0, hiringApp.jobs = d;
    }
  };
  h(0);
}
function jobSpec(c) {
  for (var d = {hnId:c.id}, e = 0; e < c.children.length; ++e) {
    jobSpecExtend(d, c.children[e]);
  }
  return d;
}
function jobSpecExtend(c, d) {
  var e = d.className;
  if ("hnuser" === e) {
    c.user = d.innerHTML;
  } else {
    if ("age" === e) {
      c.age = d.children[0].innerHTML;
    } else {
      if (3 === e.length && -1 !== "c5a,cae,c00,c9c,cdd,c73,c88".search(e)) {
        var f = d.getElementsByClassName("reply");
        Array.prototype.map.call(f, function(c) {
          c.remove();
        });
        f = d.childNodes;
        var g = !0;
        if (3 === f[0].nodeType && 1 < f[0].textContent.split("|").length) {
          c.title = [];
          c.body = [];
          for (var h = 0; h < f.length; h++) {
            n = f[h], g ? 1 === n.nodeType && "P" === n.nodeName ? (g = !1, c.body.push(n)) : c.title.push(n) : c.body.push(n);
          }
          f = c.title.map(function(c) {
            return c.textContent;
          }).join(" | ");
          var k = f.split("|").map(function(c) {
            return c.trim();
          });
...
var appHelpEntry = ["All filters are ANDed.", "RFEs welcome and can be raised <a href='https://github.com/kennytilton/matrix/issues'>here</a>. ", "GitHub source can be <a href='https://github.com/kennytilton/kennytilton.github.io/tree/main/whoishiring'>found here</a>."];
function appHelp() {
