...
function kfExpand( c, kf) {
    if ( kfExpandFinal(kf)) {
        // clg('kfsrun sees  final!', kf, kf===null, kf instanceof Model, kf instanceof Array);
        return kf;
    } else if ( typeof kf === 'function') {
        // clg('kfsrun sees function!');
        return kfExpand( c, kf(c));
    } else if (kf instanceof Array) {
        // clg('kfsrun sees array!', kf, kf.length);
        return kfExpand( c, kf.map(k => kfExpand( c, k )));
    } else {
        clg('cKids expander passed bad kf', kf, kf===null, typeof kf);
        clg('cKids c:', c.name, typeof c.md, c.md.name, c)
        return null;
        // debugger;
