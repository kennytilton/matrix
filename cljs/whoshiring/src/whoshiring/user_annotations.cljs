(ns whoshiring.user-annotations
...
(defn applied-toggle [job]
  (div {:style "display:flex; flex-wrap: wrap; align-items:center"}
    (input {:id      (str "applied?" (:hn-id job))
            ...
            :onclick (fn [e] (job-memo-swap!> job :applied not))}
      {:name :applied})
...
(defn note-toggle [job]
  (span {:style   (cF (str "margin-left:18px; cursor:pointer;color:"
         ...
         :onclick (fn [e] (mswap!> (evt-tag e) :editing not))}
    {:name    :note-toggle
...
(defn exclude-job [job]
  (span {:content "&#x20E0;"
         ...
         :onclick (fn [e]
                    (prn :click)
                    (job-memo-swap!> job :excluded not))}))

(defn editing? [me]
  (<mget (md/mxu-find-name me :note-toggle) :editing))
...
(defn note-editor [job]
  (textarea {:class       (cF (if (editing? me)
             ...
             :onchange    (fn [e] (job-memo-set!> job :notes (utl/target-val e)))}
