...
(defmacro mxfnc [& body]
  `(fn []
     (let [[~'_ set-state#] (react/useState 0)]
       (awesome.mxreact/set-state-record ~'me set-state#)
       ~@body)))

(defmacro define-composite-macro [gen-type]
  `(defmacro ~gen-type [mx-props# jsx-props# & kids#]
     `(tiltontec.model.core/make :awesome.mxreact/awesome.elt
        :sid (swap! awesome.mxreact/sid-latest inc)
        :kids (tiltontec.model.core/cFkids ~@kids#)
        :rendering (tiltontec.cell.core/cF
                     (react/createElement
                       (awesome.mxrgen/mxfnc
                         (apply react/createElement
                           (or (get {:Pressable        rn/Pressable
                                     ;:NavigationContainer (awesome.demo.navi/bottom-tab-navi :NavigationContainer)
                                     ;:Navigator           (awesome.demo.navi/bottom-tab-navi :Navigator)
                                     ;:Screen              (awesome.demo.navi/bottom-tab-navi :Screen)
                                     :Text             rn/Text
...
(define-atom-macros
  ActivityIndicator Button FlatList #_ Icon Image #_ SliderRNE Switch TextInput)

(defmacro Image [mx-props jsx-props]
  `(tiltontec.model.core/make :awesome.mxreact/awesome.elt
     :sid (swap! awesome.mxreact/sid-latest inc)
     :rendering (tiltontec.cell.core/cF
                  (react/createElement (awesome.mxrgen/mxfnc
                                         (react/createElement
                                           rn/Image
                                           ~jsx-props))))
     ~@(apply concat
